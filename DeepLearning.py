...
def build_feedforward(data):
    #Build network
    model = keras.Sequential()
    model.add(keras.layers.Embedding(input_dim=data["vocab_size"], output_dim=10, input_length=len(data["x_test"][0])))
    model.add(keras.layers.Flatten())
    model.add(keras.layers.Dense(10, activation="relu", input_dim= 10))
    model.add(keras.layers.Dense(1, activation="relu", input_dim= 10))
    #Compile the network
    model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
    return model

def build_recurrent(data):
    #Build network
    model = keras.Sequential()
    model.add(keras.layers.Embedding(input_dim=data["vocab_size"], output_dim=10, input_length=len(data["x_test"][0])))
    model.add(keras.layers.LSTM(10))
    model.add(keras.layers.Dense(5, activation="relu", input_dim = 10))
    model.add(keras.layers.Dense(1, activation="relu", input_dim = 10))
    #Compile the network
    model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
    return model
...
def train_model(data: Dict[str, Union[List[Any], np.ndarray, int]], model_type="feedforward") -> float:
    ...
    if model_type == "feedforward":
        model = build_feedforward(data)
        
    elif model_type == "recurrent":
        model = build_recurrent(data)
        
    else: 
        raise ValueError('Invalid model_type, must be either feedforward or recurrent')

    model.fit(data["x_train"], data["y_train"], epochs = 4)
    
    #Return the accuracy
    return model.evaluate(data["x_test"], data["y_test"])[1]
